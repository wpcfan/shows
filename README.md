...
| `tts-providers.js` | `require('./tools/tts-providers')` | TTS provider 注册表:`register(provider)` / `getProvider(name)` / `listProviders()`;doubao 预注册为内置 provider;`tts.js` 的 `defaultSynth` 按 `provider.name` 派发到注册表(扩展新 provider 只需 `register`) |
